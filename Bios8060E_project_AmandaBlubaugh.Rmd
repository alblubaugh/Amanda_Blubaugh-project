...
### Next, download data from GEO datasets
```{r}
...
## Aquire phenotypic data for the samples run on this microarray
...
```{r}
#Here, we ID where we saved the .txt file we got for reading into a table, notating that the file has tabular separations ("\t" for sep), that there is a header for each column (our variables; header=TRUE), and that we have row names which are the same .txt GSM# .txt files that we have read in previously.
...
In this metadata information table I have created, I have manually added color identifications in that were not part of the phenotyptic table copied to a .txt file from GEO2R.
